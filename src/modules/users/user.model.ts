...
import { User, Update } from './user.interface';
...
const userSchema = new Schema<User>(
  {
    ...
    user_metadata: {
      ...
      creditScore: { type: Number, required: false, default: 0 },
...
// Middleware to clear permissions if not admin
userSchema.pre("save", function (next) {
  if (this.role !== "admin") {
    this.permissions = []; // non-admins should not have permissions
  }
  next();
});
