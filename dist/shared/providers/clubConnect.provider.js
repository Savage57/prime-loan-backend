...
exports.ClubConnectsService = void 0;
const axios_1 = __importDefault(require("axios"));
const config_1 = require("../../config");
const exceptions_1 = require("../../exceptions");
const bill_payment_interface_1 = require("../../modules/bill-payments/bill-payment.interface");
function addEightPercent(value) {
    return String(value + (value * 0.03));
}
class ClubConnectsService {
    handleResponse(response) {
        console.log("Response:", response);
        console.log("Response data:", response.data);
        console.log("Response status code:", response.data.statuscode);
        console.log("Response status:", response.data.status);
        if (response.data && response.data.status && response.data.status !== bill_payment_interface_1.StatusCode.ORDER_RECEIVED) {
            throw new exceptions_1.APIError(Number(response.data.statuscode), bill_payment_interface_1.StatusDescriptions[response.data.status] || "System busy, try again later");
        }
...
exports.ClubConnectsService = ClubConnectsService;
